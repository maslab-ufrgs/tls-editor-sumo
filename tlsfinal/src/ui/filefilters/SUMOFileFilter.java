...
public class SUMOFileFilter extends javax.swing.filechooser.FileFilter {
	...
	public boolean accept(File f) {
		...
		// ALTEREI AQUI: if ((f.getName().endsWith(".net.xml")) && (new File(f.getAbsolutePath().replaceFirst(".net.", ".edg.")).exists()) && (new File(f.getAbsolutePath().replaceFirst(".net.", ".nod.")).exists())) {
		if ((f.getName().endsWith(".net.xml")) ){
                    return true;
		} else {
			return false;
		}
